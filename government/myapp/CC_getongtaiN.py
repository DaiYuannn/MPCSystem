...
def MatrixDot(A,B,ms,o,ns,mods):#计算矩阵乘法,A是m*o的矩阵，B是o*n的矩阵
    ...
    for i in range(0,ms):
        for j in range(0,ns):
            for k in range(0,o):
                Matrix[i][j]=(Matrix[i][j]+A[i][k]*B[k][j]) % mods
...
def CreatShard(Phard,A,B,Dhard,Deta,N,mods):
    ...
    right1 = MatrixDot(Phard, B, N, N, N,mods)
    right2 = MatrixDot(Dhard, Deta, N, N, N,mods)
    right=MatrixAdd(right1,right2,N,N,mods)
...
def CreatSsoft(Psoft,A,B,Dsoft,Deta,N,n,mods):
    ...
    for i in range(n):
        ...
        right1 = MatrixDot(Psoft[i], B, N, N, N,mods)
        right2 = MatrixDot(Dsoft[i], Deta, N, N, N,mods)
        right = MatrixAdd(right1, right2, N,N,mods)
...
def lattice_decryption(ciphertexts, Deta, A, B, N, mods, qq):
    ...
    temp1=list()
    temp1.append(CU)
    e = MatrixDot(temp1,matrix_inv(A,N,mods),1,N,N,mods)
    e = MatrixDot(e,B,1,N,N,mods)
    temp2 = list()
    temp2.append(CD)
    e = MatrixSub(temp2,e,1,N,mods)
    # e = MatrixModP(e, 1, N, p)
    e = MatrixDot(e, matrix_inv(Deta,N, mods), 1, N, N,mods)
    #e = MatrixModP(e, 1, N,mods)
